...
ROOTFS_SIZE=`wc -c < $PACKAGE_BASE_DIR/rootfs_$PKG_NAME.cpio.gz`
if [ $ROOTFS_SIZE -gt $(($IMG_SZ * 1024 * 1024)) ]; then
    echo rootfs image size $ROOTFS_SIZE exceeds limit $IMG_SZ MB
    exit 1
fi
